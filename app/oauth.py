...
def get_oauth_client(service, key, secret, callback_url):
  ...
  elif service == TRADEKING:
    return TKClient(key, secret, callback_url)
  else:
...
class TKClient(OAuthClient):
      """Tradeking Client.

      A client for talking to the Tradeking API using OAuth as the
      authentication model.
      """
      ...
      def get_authorization_url(self):
        ...
        token = self._get_auth_token()
        return "https://developers.tradeking.com/oauth/authorize?oauth_token=%s" % token
